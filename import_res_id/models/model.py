...
class ResIdMixin(models.AbstractModel):
    ...
    res_external_id = fields.Char(
        compute="_compute_get_external_id",
        inverse="_inverse_set_external_id",
        string="Related object external id", help="Related external id")

    @api.multi
    def _compute_get_external_id(self):
        rec_per_model_res_id = defaultdict(list)
        for rec in self:
            rec_per_model_res_id[
                (rec[self._model_field], rec[self._res_id_field])].append(rec)
        ...
        for model, group in groupby(self, lambda x: x[self._model_field]):
            domain.extend([
                '&',
                ('model', '=', model),
                ('res_id', 'in', [g[self._res_id_field] for g in group])])
        domain = ['|'] * (len(domain) / 3 - 1) + domain

        for external_id in self.env['ir.model.data'].search(domain):
            for record in rec_per_model_res_id[(
                    external_id.model, external_id.res_id)]:
                record.res_external_id = "%s.%s" % (
                    external_id.module, external_id.name)

    @api.multi
    def _inverse_set_external_id(self):
        xml_ids = [
            _extract_xml_id(rec.res_external_id)
            for rec in self if rec.res_external_id]
        ...
        for module, group in groupby(xml_ids, lambda x: x[0]):
            domain.extend([
                '&',
                ('module', '=', module),
                ('name', 'in', [g[1] for g in group])])
        domain = ['|'] * int(len(domain) / 3 - 1) + domain

        for external_id in self.env['ir.model.data'].search(domain):
            for record in rec_per_xml_id[(
                    external_id.module, external_id.name)]:
